    ...
    if os.path.exists(FONT_FILENAME):
        ...
        try:
            ...
            if os.path.exists(system_path):
                ...
                print("Warning: Nirmala.ttf not found. Hindi will not render.")
                return None, "Helvetica" # Fallback
        except:
            pass
...
def draw_watermark(c, width, height):
    ...
    if os.path.exists(WATERMARK_IMG):
        c.saveState()
        c.setFillAlpha(0.15) # 15% Opacity (Very light)
        wm_size = 350
...
def draw_header(c, width, height, page_text=""):
    """Draws Logos, Title, and Exam Details"""
    # Logos
    logo_size = 65
    margin = 25
    ...
    if page_text == "Page 1":
        ...
        # Helper to measure text and draw lines
        c.drawString(30, box_top - 17, "नाम: _____________________________   रोल नं: _______   दिनांक: _______")
        
        # Max Marks & Questions
        c.setFont(FONT_NAME, 10)
        c.setLineWidth(1.5) # Thicker line
        c.line(20, box_top - 35, width - 20, box_top - 35) # Separator
...
def draw_question_box(c, x, y, w, h, title, marks):
    ...
    # 1. Draw Rounded Rect (Border)
    c.setLineWidth(1)
    c.setStrokeColor(colors.black)
    c.setFillColor(colors.white)
    c.roundRect(x, y, w, h, radius, stroke=1, fill=1)
    
    # 2. Draw Header Background (Grey)
    # To do this cleanly with rounded top corners only is complex in reportlab standard.
    # We will draw a rounded rect filled grey, then cover the bottom half with white square.
    header_h = 20
    c.saveState()
    
    # Clip to the box shape so the header doesn't spill
    p = c.beginPath()
    ...
    # Draw Title Text
    c.setFillColor(colors.black)
    c.setFont(FONT_NAME, 10) # Bold-ish look via font
...
def draw_vertical_math(c, x, y, num1, op, num2):
    """Aligns math problems:   24
                             + 13
    """
    c.setFont(FONT_NAME, 14)
    # Draw first number aligned right
